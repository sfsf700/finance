class RecordsController < ApplicationController
  ...
  def index
    ...
    if simplecalendar != nil
      ...
      @select_income = Record.where(status: 2).where(user_id: current_user.id).sum(:price) ## 収入
      @select_spending = Record.where(status: 1).where(user_id: current_user.id).sum(:price) ## 支出
    end
